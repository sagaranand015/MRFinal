	        ...
	        <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12 main-div upload-div">
	        	...
	        	<?php 
                    ...
                    if(isset($_FILES["fileAnnualReport"]) && $_FILES["fileAnnualReport"]["error"]== UPLOAD_ERR_OK)
                    {
                        ...
                        $fileName           = strtolower($_FILES['fileAnnualReport']['name']);
                        ...
                        $course = "-1";
                        $annualReportName = "-1";
                        if(isset($_POST["txtAnnualReportName"])) {
                            $annualReportName = $_POST["txtAnnualReportName"];
                        }
                        if(isset($_POST["course-annual-report"])) {
                            $course = $_POST["course-annual-report"];
                        }
                        
                        if($course == "-1" || $annualReportName == "") {
                            echo "<h3 class='page-header'>Upload failed</h3><p>Looks like the parameters for Annual Report upload are not correct. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                        }
                        else {
                            if(move_uploaded_file($_FILES['fileAnnualReport']['tmp_name'], $uploadDirectory.$newFileName )) {
                                // put url of the calender file here.
                                $register = RegisterAnnualReportUrl($course, $annualReportName, $uploadDirectory.$newFileName);
                                if($register == "1") {
                                    echo "<h3 class='page-header'>Upload Success</h3><p>Your Annual Report upload is successful.</p>";
                                }
                                else if($resp == "-1") {
