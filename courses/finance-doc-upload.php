	        ...
	        <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12 main-div upload-div">
	        	...
	        	<?php 
                    ...
                    if(isset($_FILES["fileFinanceDoc"]) && $_FILES["fileFinanceDoc"]["error"]== UPLOAD_ERR_OK)
                    {
                        ...
                        $fileName           = strtolower($_FILES['fileFinanceDoc']['name']);
                        ...
                        $course = "-1";
                        $financeDocName = "-1";
                        if(isset($_POST["txtFinanceDocName"])) {
                            $financeDocName = $_POST["txtFinanceDocName"];
                        }
                        if(isset($_POST["course-finance-doc"])) {
                            $course = $_POST["course-finance-doc"];
                        }
                        
                        if($course == "-1" || $financeDocName == "") {
                            echo "<h3 class='page-header'>Upload failed</h3><p>Looks like the parameters for finance Document upload are not correct. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                        }
                        else {
                            if(move_uploaded_file($_FILES['fileFinanceDoc']['tmp_name'], $uploadDirectory.$newFileName )) {
                                //put url of the calender file here.
                                $register = RegisterFinanceDocUrl($course, $financeDocName, $uploadDirectory.$newFileName);
                                if($register == "1") {
                                    echo "<h3 class='page-header'>Upload Success</h3><p>Your Finance Document upload is successful.</p>";
                                }
                                else if($resp == "-1") {
