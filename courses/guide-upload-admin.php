	        ...
	        <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12 main-div upload-div">
	        	...
	        	<?php 
                    ...
                    if(isset($_FILES["fileGuide"]) && $_FILES["fileGuide"]["error"]== UPLOAD_ERR_OK)
                    {
                        ...
                        $fileName           = strtolower($_FILES['fileGuide']['name']);
                        ...
                        $course = "-1";
                        $guideName = "-1";
                        if(isset($_POST["txtGuideName"])) {
                            $guideName = $_POST["txtGuideName"];
                        }
                        if(isset($_POST["course-guide"])) {
                            $course = $_POST["course-guide"];
                        }

                        if($course == "-1" || $guideName == "-1") {
                            echo "<h3 class='page-header'>Upload failed</h3><p>Looks like the parameters for guide upload are not correct. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                        }
                        else {
                            if(move_uploaded_file($_FILES['fileGuide']['tmp_name'], $uploadDirectory.$newFileName )) {
                                // put url of the calender file here.
                                $register = RegisterGuideUrl($course, $guideName, $uploadDirectory.$newFileName);
                                if($register == "1") {
                                    echo "<h3 class='page-header'>Upload Success</h3><p>Your Guide upload is successful.</p>";
                                }
                                else if($resp == "-1") {
