...
function SendAdvancedQuizResponseMail($quizId, $assId, $menteeId, $menteeEmail, $givenAns, $ans, $correct) {
	...
	try {
		...
		$msg .= "<b>You will be notified about your score once the admin evaluates your submitted Quiz.</b><br /><br />";
...
// this is the function for registering the annual report url to the database table.
function RegisterFinanceDocUrl($courseID, $financeDocName, $url) {
	$resp = "-1";
	try {
		$query = "insert into FinanceDocument(FinanceDocumentName, FinanceDocument, FinanceDocumentCourse) values('$financeDocName', '$url', '$courseID')";
		$rs = mysql_query($query);
		if(!$rs) {
			$resp = "-1";
		}
...
// this is the function for registering the annual report url to the database table.
function RegisterAnnualReportUrl($courseID, $annualReportName, $url) {
	$resp = "-1";
	try {
		$query = "insert into AnnualReport(AnnualReportName, AnnualReport, AnnualReportCourse) values('$annualReportName', '$url', '$courseID')";
		$rs = mysql_query($query);
		if(!$rs) {
			$resp = "-1";
		}
