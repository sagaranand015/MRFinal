    	...
    	<div class="row">
	        ...
	        <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12 main-div upload-div">
	        	...
	        	<?php 
                    ...
                    if(isset($_FILES["fileAssignmentOffTopic"]) && $_FILES["fileAssignmentOffTopic"]["error"]== UPLOAD_ERR_OK)
                    {
                        ############ Edit settings ##############
                        $uploadDirectory    = 'uploads/assignmentOffTopic/'; //specify upload directory ends with / (slash)
                        ##########################################
                        
                        $fileName           = strtolower($_FILES['fileAssignmentOffTopic']['name']);
                        ...
                        $courseId = "-1";
                        $assId = "-1";
                        if(isset($_POST["course-list-ass-off-topic"])) {
                            $courseId = $_POST["course-list-ass-off-topic"];
                        }
                        if(isset($_POST["assignment-list-ass-off-topic"])) {
                            $assId = $_POST["assignment-list-ass-off-topic"];
                        }
                        
                        if($courseId == "-1" || $assId == "-1") {
                            echo "<h3 class='page-header'>Upload failed</h3><p>Looks like the parameters for Off Topic Read upload are not correct. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                        }
                        else {
                            if(move_uploaded_file($_FILES['fileAssignmentOffTopic']['tmp_name'], $uploadDirectory.$newFileName )) {
                                // save the link to the database.
                                $register = RegisterAssignmentOffTopic($assId, $courseId, $uploadDirectory.$newFileName);
                                if($register == "1") {
                                    echo "<h3 class='page-header'>Upload Success</h3><p>Your Assignment Off Topic Read upload is successful.</p>";   
                                }
                                else {
