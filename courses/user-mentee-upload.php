    	...
    	<div class="row">
	        ...
	        <div class="col-lg-10 col-lg-offset-1 col-md-10 col-md-offset-1 col-sm-12 col-xs-12 main-div upload-div">
	        	...
	        	<?php 
                    ...
                    if(isset($_FILES["fileAddMentee"]) && $_FILES["fileAddMentee"]["error"]== UPLOAD_ERR_OK)
                    {
                        ############ Edit settings ##############
                        $uploadDirectory    = 'uploads/addUser/'; //specify upload directory ends with / (slash)
                        ##########################################
                        
                        $fileName           = strtolower($_FILES['fileAddMentee']['name']);
                        ...
                        if(isset($_POST["add-mentee-excel-course"])) {
                            $course = $_POST["add-mentee-excel-course"];
                        }
                        if(isset($_POST["add-mentee-excel-organ"])) {
                            $organ = $_POST["add-mentee-excel-organ"];
                        }

                        // echo "The params are: " . $email . " --> " . $id . " --> " . $course . " --> " . $organ . " <br /><br />";
                        ...
                        if($organ == "-1" || $course == "-1") {
                            ...
                            echo "<h3 class='page-header'>Upload failed</h3><p>Looks like you have not logged in properly. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                        }
                        else {
                                ...
                                try {
                                    ...
                                    for ($row = 1; $row <= $highestRow; $row++) {
                                        ...
                                        foreach($rowData[0] as $k=>$v) {
                                            // $resp .= " <br /><br />Row: ".$row."- Col: ".($k+1)." = ".$v."<br />";
                                            $res = AddToUsers($v, "D");
                                            if($res == "1") {
                                                $i = $i + 1;

                                                $reslt = AddToMenteeTable($organ, $course, $v, "Mentee");   // add to the Mentee table
                                                if($reslt != "-1") {
                                                    $resp = AddToMenteeCoursesTable($reslt, $v, $course);   // add to the MenteeCourses table.
                                                    if($resp == "1") {
                                                        SendNewUserMail($v);
                                                    } else {
                                                        $l = $l + 1;
                                    ...
                                    $response = $i . " user(s) have been Added. <br />Not Added: " . $userDefault . "<br /><br />" . " Mentee(s) have also been added. <br />Not Added: " . $menteeDefault . ".<br /><br />";
                                    echo "<h3 class='page-header'>Upload Success</h3><p>Your Mentee Excel upload is successful. " . $response . "</p>";                                    
                                } 
                                catch (Exception $e) {
                                    echo "<h3 class='page-header'>Upload failed</h3><p>Looks like we could not read the uploaded excel file. Please try again or contact us at: <code>tech@mentored-research.com</code></p>";
                                }
                            }
                            else {
