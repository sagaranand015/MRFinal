...
if(isset($_FILES["fileAddMentee"]) && $_FILES["fileAddMentee"]["error"]== UPLOAD_ERR_OK)
{
	...
	*/
	...
	if ($_FILES["fileAddMentee"]["size"] > 5242880) {
		die("File size is too big!");
	}
	
	//allowed file type Server side check
	switch(strtolower($_FILES['fileAddMentee']['type']))
	{
		//allowed file types
	...
	$File_Name          = strtolower($_FILES['fileAddMentee']['name']);
			...
			try {
				...
				for ($row = 1; $row <= $highestRow; $row++) {
				    ...
				    foreach($rowData[0] as $k=>$v) {
				    	...
				    	$resp = AddToTable($v, $organ, $course, "Mentee");
				    	if($resp == "1") {
				    		$k = $k + 1;
				    	}
				    	else {
				...
				$response = $i . " user(s) have been Added. <br />Not Added: " . $userDefault . "<br /><br />" . " Mentee(s) have also been added. <br />Not Added: " . $menteeDefault . ".<br /><br /> Please wait while we reload the page for you.";
				die($response);
